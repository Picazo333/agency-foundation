---
status: review
owner: brand
updated: 2026-09-16
authority: workbench
depends_on:
  - ../CREATIVE_DIRECTION_BRIEF.md
  - ../REFERENCE_CANON.md
  - ../ANTI_CANON.md
  - ../reference-research/PHASE_2_REFERENCE_RESEARCH.md
  - ../reference-research/SOURCE_REGISTRY.md
---
# DIVINIVID Reference Atlas

## State
**ACTIVE — Phase 3 current gate.**

Phase 1 Creative Direction and Phase 2 Expanded Reference Research are closed with `PASS`. This folder now becomes the active curation workspace.

## Required boards
1. `01_DIVINE_ILLUMINATION`
2. `02_GROTESQUE_CORPOREAL`
3. `03_DARKNESS_REVELATION`
4. `04_ORDER_KNOWLEDGE`
5. `05_RECONCILIATION`
6. `ANTI_ATLAS`

## Input universe
Use:
- Clean Reference Canon;
- Phase 2 promoted/support candidates;
- Phase 2 authoritative Source Registry;
- Anti-Canon for rejection boundaries.

Candidate status is not automatic board admission. Phase 3 exists to curate and cut.

## Board procedure
For each positive board:
1. write the emotional/analytical target before final selection;
2. begin from the real-source candidate pool, not generated project work;
3. maintain broad consideration, then cut ruthlessly to **8–16 unique contributors**;
4. label every survivor `INSPIRATION` or `EXECUTION`;
5. cover color/tone, material/texture, light/shadow, composition/space and body/human presence when relevant;
6. record source metadata plus `EXTRACT / DO NOT EXTRACT` and rights/provenance state;
7. limit single-author/film domination unless the board explicitly studies that source;
8. assemble in **Figma** rather than asking a generative image model to synthesize the board;
9. run Visual Asset Curation on the set;
10. run the eight transversal quality gates;
11. obtain explicit human `PASS / MUTATE / KILL`.

## Source rule
Only real/source references belong on these boards. Project-generated imagery is excluded unless later explicitly human-approved as an execution reference, and even then it cannot replace the underlying source record.

## Anti-Atlas
Use the Anti-Atlas to make rejected territories concrete. It must explain *why* each example conflicts with DIVINIVID rather than functioning as a taste-shaming board.

Priority anti-directions include:
- pseudo-luxury black/gold;
- generic AI gothic;
- occult/tarot branding;
- statue + serif + black;
- brutalist agency template;
- dark academia;
- fantasy/game medieval;
- cyber-neon tech;
- colorless pseudo-medievalism;
- modern medical anatomy branding.

## Deliverable requirement
Each final board must include:
- an art-direction statement;
- 8–16 curated references;
- source/provenance list;
- `EXTRACT / DO NOT EXTRACT` rationale;
- what master tension(s) it demonstrates;
- why the board is distinct from the other four positive boards;
- explicit human gate decision.

No KIROGRAF route-production 5-Up begins until this Phase 3 gate closes.
